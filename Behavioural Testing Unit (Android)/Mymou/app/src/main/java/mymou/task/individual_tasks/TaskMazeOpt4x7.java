...
import org.json.JSONObject; // Use your preferred JSON library
...
public class TaskMazeOpt4x7 extends Task {
    ...
    // Maze data and loader
    private MazeData mazeData;
    private MazeLoader loader;
    // Keeps track of CSV row indices (data rows; header is row 0) already sampled.
    private Set<Integer> sampledIndices = new HashSet<>();
    private Set<String> sampledMazeIDs = new HashSet<>();
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        // Create an instance of MazeLoader
        loader = new MazeLoader(getContext());

        // Get maze grid parameters
        GridParameters params = calculateGridParameters(cueSize);
        ...
        // Sample and initiate a maze
         assignObjects();

        // Display the fixation cross (it will stay indefinitely until tapped).
        //displayFixationCross(parentLayout);
    }
    ...
    public class GridParameters {
        ...
        public GridParameters(int gridScale, int offsetX, int offsetY, int gridRows, int gridCols) {
    ...
    public GridParameters calculateGridParameters(int cueSize) {
        ...
        int margin = cueSize / 2 + 100;

        // Calculate usable width and height
        int usableWidth = screenSize.x - (2 * margin);
        int usableHeight = screenSize.y - (2 * margin);

        // Define minimum and maximum grid scale
        int minGridScale = cueSize + (cueSize / 100) * 75;
        int maxGridScale = Math.min(usableWidth, usableHeight) / 5;

        // Use a dynamic percentage for grid scale (for example, 45%)
        float scalePercentage = 45f;
        int gridScale = minGridScale + (int)((maxGridScale - minGridScale) * (scalePercentage / 100f));

        // Define the grid size (could be parameters if you like)
        int gridRows = 7;
        int gridCols = 4;

        // Calculate offsets to center the grid within the usable area
        int offsetX = (screenSize.x - usableWidth) / 2;
        int offsetY = (screenSize.y - usableHeight) / 2;

        return new GridParameters(gridScale, offsetX, offsetY, gridRows, gridCols);
    }

    /** Call this method from your startTrial() method.
    private void displayFixationCross(final ViewGroup parentLayout) {
        // Set the background color to black.
        parentLayout.setBackgroundColor(getResources().getColor(android.R.color.black));

        // Create the fixation cross button.
        final Button fixationButton = new Button(getContext());
        fixationButton.setText("+");           // Use "+" as the fixation cross.
        fixationButton.setTextSize(48);
        fixationButton.setTextColor(Color.WHITE);
        fixationButton.setBackgroundColor(Color.TRANSPARENT); // Remove default background.

        // Set layout parameters to center the button.
        // (Assuming parentLayout is a RelativeLayout; adjust accordingly for other layouts.)
        parentLayout.LayoutParams params = new parentLayout.LayoutParams(
                parentLayout.LayoutParams.WRAP_CONTENT,
                parentLayout.LayoutParams.WRAP_CONTENT);
        params.addRule(parentLayout.CENTER_IN_PARENT, parentLayout.TRUE);
        fixationButton.setLayoutParams(params);

        // Set an OnClickListener to remove the button and call assignObjects() when pressed.
        fixationButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Remove the fixation cross from the parent layout.
                parentLayout.removeView(fixationButton);
                // Call the method to assign objects.
                assignObjects();
            }
        });

        // Add the fixation button to the parent layout.
        parentLayout.addView(fixationButton);
    }*/
    ...
    public void startTrial() {
        ...
        logEvent("Starting trial", callback);
    ...
    private List<Point> buildGrid(int rows, int cols, int gridScale, int offsetX, int offsetY) {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            for (int col = 0; col < cols; col++) {
                int x = offsetX + col * gridScale; // Calculate x position
                int y = offsetY + invertedRow * gridScale; // Calculate y position with inversion
                positions.add(new Point(x, y)); // Add to list of positions
            }
        }
        return positions;
        ...
        node.setOnClickListener(v -> {
            if (isNeighbor(currentPosition, position)) {
                ...
                if (position.equals(goal)) {
                    // Disable all node clickability
                    disableAllNodeClickability();

                    // Give reward and end trial
    ...
    private void disableAllNodeClickability() {
    ...
    // A simple class to hold a maze configuration.
    public class MazeData {
        public String mazeJson;
        public Point start;
        public Point goal;

        public MazeData(String mazeJson, Point start, Point goal) {
    ...
    public class MazeLoader {
        ...
        public MazeLoader(Context context) {
            this.context = context;
        }

        // Load persisted maze IDs from SharedPreferences.
        private void loadSampledMazeIDs() {
            SharedPreferences prefs = context.getSharedPreferences("mazePrefs", Context.MODE_PRIVATE);
            String idsString = prefs.getString("sampledMazeIDs", "");
            sampledMazeIDs.clear();
            if (!idsString.isEmpty()) {
                String[] parts = idsString.split(",");
                for (String part : parts) {
                    sampledMazeIDs.add(part.trim());
                }
            }
        }

        // Save sampled maze IDs to SharedPreferences.
        private void saveSampledMazeIDs() {
            SharedPreferences prefs = context.getSharedPreferences("mazePrefs", Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = prefs.edit();
            StringBuilder sb = new StringBuilder();
            for (String id : sampledMazeIDs) {
                sb.append(id).append(",");
            }
            String idsString = sb.length() > 0 ? sb.substring(0, sb.length() - 1) : "";
            editor.putString("sampledMazeIDs", idsString);
            editor.apply();
        }


        /**
         * Reads the CSV file from res/raw and randomly samples an unsampled maze configuration.
         *
         * @param gridScale Scale factor used for converting row/col to pixel positions.
         * @param offsetX   X offset for the grid.
         * @param offsetY   Y offset for the grid.
         * @return MazeData object containing the maze JSON, start, and goal or null if all rows were used.
         */



        public MazeData sampleMazeData(int gridScale, int offsetX, int offsetY, int gridRows) {
            // Load persisted maze IDs first.
            loadSampledMazeIDs();

            List<Integer> unsampledRowIndices = new ArrayList<>();
            List<String[]> allRows = new ArrayList<>();

            try {
                InputStream inputStream = context.getResources().openRawResource(R.raw.optimised_mazes_easy);
            ...
            // Iterate over rows (skip header at index 0) and compute a maze ID from the JSON.
            for (int i = 1; i < allRows.size(); i++) {
                String mazeJson = allRows.get(i)[0];
                // Compute a unique ID. Here we use hashCode, but you could use a more robust hash if desired.
                String mazeID = Integer.toString(mazeJson.hashCode());
                if (!sampledMazeIDs.contains(mazeID)) {
                    unsampledRowIndices.add(i);
                }
            }

            if (unsampledRowIndices.isEmpty()) {
                // All mazes have been sampled.
                parentLayout.setBackgroundColor(getResources().getColor(android.R.color.black));
                disableAllNodeClickability();
                return null;
            }
            ...
            int selectedRowIndex = unsampledRowIndices.get(rand.nextInt(unsampledRowIndices.size()));

            // Mark the selected maze as sampled.
            String selectedMazeJson = allRows.get(selectedRowIndex)[0];
            String selectedMazeID = Integer.toString(selectedMazeJson.hashCode());
            sampledMazeIDs.add(selectedMazeID);
            saveSampledMazeIDs();

            // Process the CSV row to extract mazeJson, start, and goal.
            String[] parts = allRows.get(selectedRowIndex);
            if (parts.length < 3) {
                return null;
            }
            String mazeJson = parts[0];

            // Process the start field (expected format "row:col")
            String startStr = parts[1].trim().replace("\"", "");
            String[] startParts = startStr.split(":");
            int startRow = Integer.parseInt(startParts[1].trim());
            int startCol = Integer.parseInt(startParts[0].trim());

            // Process the goal field (expected format "row:col")
            String goalStr = parts[2].trim().replace("\"", "");
            String[] goalParts = goalStr.split(":");
            int goalRow = Integer.parseInt(goalParts[1].trim());
            int goalCol = Integer.parseInt(goalParts[0].trim());

            // Invert the Y coordinate by using gridRows (if needed)
            Point start = new Point(startCol * gridScale + offsetX, (gridRows - 1 - startRow) * gridScale + offsetY);
            goal = new Point(goalCol * gridScale + offsetX, (gridRows - 1 - goalRow) * gridScale + offsetY);

           // logEvent("start point: " + start, callback);
           // logEvent("goal point: " + goal, callback);
            return new MazeData(mazeJson, start, goal);
        }

        public void populateGridFromMazeData(MazeData mazeData, int rows, int cols, int gridScale, int offsetX, int offsetY, boolean sparse) {
            // Build grid positions (this function should create a list of Points).
            List<Point> positions = buildGrid(rows, cols, gridScale, offsetX, offsetY);

            // Create nodes for each grid position (y-axis inverted to match nx graph default).
            ...
            // Use the start and goal from the mazeData.
            Point start = mazeData.start;
            Point goal = mazeData.goal;

            // Highlight valid action nodes and the goal.
            ...
            // Set current position to start and place the blue circle.
            currentPosition = start;
            createBlueCircle(start);

            // Log start and goal node positions.
            int start_row = (start.y - offsetY) / gridScale;
            int start_col = (start.x - offsetX) / gridScale;
            String startNodeId = start_row + "," + start_col;
            logEvent("startNode:" + startNodeId, callback);

            int goal_row = (goal.y - offsetY) / gridScale;
            int goal_col = (goal.x - offsetX) / gridScale;
            String goalNodeId = goal_row + "," + goal_col;
            logEvent("goalNode:" + goalNodeId, callback);

            // Parse the maze JSON to extract the edge list.
            // Implement parseMazeEdges() based on your maze JSON structure.
            List<Edge> mazeEdges = parseMazeEdges(mazeData.mazeJson, positions, 7, 4);

            // If you want to alter the connectivity (e.g., sparse vs. fully connected), you can adjust here.
            // For this example we use the mazeEdges directly.
            edges = mazeEdges;
            ...
            logEvent("nodes:" + nodes, callback);

            // Optionally, highlight nodes again.
            highlightNodes(currentPosition, goal);
        }

        public List<Edge> parseMazeEdges(String mazeJson, List<Point> positions, int numRows, int numCols) {
            List<Edge> edgeList = new ArrayList<>();
            try {
                JSONObject json = new JSONObject(mazeJson);
                JSONArray nodesArray = json.getJSONArray("nodes");
                // Build a mapping from "col:row" to index in positions.
                Map<String, Integer> nodeKeyToIndex = new HashMap<>();
                for (int i = 0; i < nodesArray.length(); i++) {
                    JSONObject nodeObj = nodesArray.getJSONObject(i);
                    // Read the explicit row and col from the JSON.
                    int nodeRow = nodeObj.getInt("row");
                    int nodeCol = nodeObj.getInt("col");
                    // Create key as "col:row"
                    String key = nodeCol + ":" + nodeRow;
                    // Compute the index into positions (assuming buildGrid created positions in row-major order)
                    int index = nodeRow * numCols + nodeCol;
                    nodeKeyToIndex.put(key, index);
                }

                JSONArray links = json.getJSONArray("links");
                for (int i = 0; i < links.length(); i++) {
                    JSONObject link = links.getJSONObject(i);
                    // The source and target fields are arrays, e.g., [col, row]
                    JSONArray sourceArray = link.getJSONArray("source");
                    JSONArray targetArray = link.getJSONArray("target");

                    int sourceCol = sourceArray.getInt(0);
                    int sourceRow = sourceArray.getInt(1);
                    int targetCol = targetArray.getInt(0);
                    int targetRow = targetArray.getInt(1);

                    String sourceKey = sourceCol + ":" + sourceRow;
                    String targetKey = targetCol + ":" + targetRow;

                    Integer sourceIndex = nodeKeyToIndex.get(sourceKey);
                    Integer targetIndex = nodeKeyToIndex.get(targetKey);

                    if (sourceIndex != null && targetIndex != null &&
                            sourceIndex < positions.size() && targetIndex < positions.size()) {
                        Point sourcePoint = positions.get(sourceIndex);
                        Point targetPoint = positions.get(targetIndex);
                        edgeList.add(new Edge(sourcePoint, targetPoint));
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        public void populateGrid(int rows, int cols, int gridScale, int offsetX, int offsetY,
            ...
            if (sparse) {
                ...
                removeRandomEdges(3); // 7 Remove 3 edges
            }
        ...
        private void assignObjects() {
            ...
            // Sample Maze data
            mazeData = loader.sampleMazeData(gridScale, offsetX, offsetY, gridRows);

            if (mazeData != null) {
                // Populate the grid using the loaded maze data.
                // The last parameter 'false' indicates non-sparse (fully connected) grid.
                loader.populateGridFromMazeData(mazeData, gridRows, gridCols, gridScale, offsetX, offsetY, false);
            } else {
                // Handle the case where no maze data is available (all rows sampled)
                logEvent("No more maze data available to sample.", callback);
                log_trial_outcome(true); // logs previous trials
                callback.commitTrialDataFromTask_("SUCCESSFUL_TRIAL");
                resetTrial();
                // int one_week =  1000*60*60*24*7; // 1 week in milliseconds
                //showInterTrialInterval(one_week);
            }
